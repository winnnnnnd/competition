...
from ..domain.identifiers import stable_hash
from ..domain.models import DocumentElement, ElementType, SourceLocator
from ..infrastructure.object_storage import ObjectStorage
from ..infrastructure.vector_store import MilvusVectorStore
...
class EmbeddingActor:
    def __init__(self, model_value: Dict[str, Any], profile: str):
        ...
        if self.backend == "qwen_mindspore":
            from ..model_backends.mindspore.embedding import QwenEmbeddingModel
            ...
            from ..model_backends.mindnlp import MindNLPEmbeddingBackend

            self.model = MindNLPEmbeddingBackend(self.config.embedding_model)
        else:
...
class RerankerActor:
    def __init__(self, model_value: Dict[str, Any], profile: str):
        ...
        self.model = None
        if self.backend == "qwen_mindspore":
            from ..model_backends.mindspore.reranker import QwenRerankerModel
...
class LLMActor:
    def __init__(self, model_value: Dict[str, Any], profile: str):
        ...
        if self.backend == "qwen_mindspore":
            from ..model_backends.mindspore.causal_lm import QwenCausalLM
            ...
            from ..model_backends.mindnlp import MindNLPLLMBackend

            self.tokenizer = None
            self.model = MindNLPLLMBackend(self.config.llm_model)
        else:
