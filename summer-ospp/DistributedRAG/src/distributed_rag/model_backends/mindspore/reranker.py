"""MindSpore implementation of the Qwen reranker backend."""
import os
import json
import math
import numpy as np
...
class QwenModel(nn.Cell):
    ...
    def construct(self, input_ids: Tensor, attention_mask: Tensor):
        ...
        for layer in self.layers:
            h = layer(h, positions, causal_mask)
        return self.norm(h)

class RerankerModel(nn.Cell):
    def __init__(self, config: QwenConfig):
        super().__init__()
        self.model = QwenModel(config)
        self.score_head = nn.Dense(config.hidden_size, 1, has_bias=False)
    def construct(self, input_ids: Tensor, attention_mask: Tensor):
        last_hidden_state = self.model(input_ids=input_ids, attention_mask=attention_mask)
        cls_representation = last_hidden_state[:, 0]
        score = self.score_head(cls_representation)
        return score

class QwenRerankerModel:
    def __init__(self, model_name_or_path: str, device: str = "auto"):
        self._set_device(device)
        print(f"Step 1: Loading model from '{model_name_or_path}'...")
        self.local_path = self._get_local_path(model_name_or_path)
        print(f"Model files are located at: {self.local_path}")
        ...
        print("\nStep 4: Initializing model and loading weights...")
        self.model = self._initialize_and_load_weights()
        self.model.set_train(False)
        print("✅ Reranker model is ready for inference.")
    ...
    def _initialize_and_load_weights(self) -> RerankerModel:
        model = RerankerModel(self.config)
        param_dict = {}
            ...
            if f.endswith(".safetensors"):
                ...
                for name, pt_tensor in torch_dict.items():
                    if name == "embed_tokens.weight":
                        name = "model.embed_tokens.embedding_table"
                    elif name == "lm_head.weight":
                        name = "score_head.weight"
                    else:
                        name = "model." + name
                    param_dict[name] = Parameter(Tensor(pt_tensor.to(torch.float32).numpy()), name=name)

        load_param_into_net(model, param_dict, strict_load=False)
        model.to_float(ms.float32)
        return model

    def compute_score(self, sentence_pairs: List[Tuple[str, str]], max_length: int = 512) -> List[float]:

        inputs = self.tokenizer(sentence_pairs, padding=True, truncation=True, return_tensors="np", max_length=max_length)
        input_ids = Tensor(inputs['input_ids'])
        attention_mask = Tensor(inputs['attention_mask'])

        scores = self.model(input_ids=input_ids, attention_mask=attention_mask)
        return scores.asnumpy().flatten().tolist()
