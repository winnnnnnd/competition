...
from rag_core.config import AppConfig, RayConfig
from rag_core.ids import make_chunk_id
from rag_core.models import SourceLocator
from rag_core.pipeline import IngestionPipeline
...
def test_timeout_path_cancels_outstanding_ray_work(monkeypatch):
    ...
    fake_ref = object()
    monkeypatch.setattr("rag_core.pipeline.ray.wait", lambda *args, **kwargs: ([], [fake_ref]))
